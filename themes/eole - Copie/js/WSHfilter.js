...
var properties = {
    ...
    artistsTFsortingdefault: window.GetProperty("Sort Order - ARTIST", "$meta(artist,0) | %date% | %album% | %discnumber% | %tracknumber% | %title%"),
...
oFilterBox = function() {
	...
	this.on_init = function() {
		this.inputbox = new oInputbox(cFilterBox.w, cFilterBox.h, "", "", colors.normal_txt, 0, 0, colors.filter_selectedtext_bg, g_sendResponse, "brw");
    ...
    this.reset_layout = function() {
        ...
        this.inputbox.backselectioncolor = colors.filter_selectedtext_bg;
	...
	this.draw = function(gr, x, y) {
		...
		for(i = this.items_txt.length-1; i >= 0; i--) {
			...
			if(this.items_txt[i].length==1){
				...
				gr.GdiDrawText(this.items_txt[i], g_font.min1, (i==this.activeItem || i==this.hoverItem)?colors.normal_txt:btn_inactive_txt, this.items_x[i], this.txt_top_margin, this.items_width[i], this.h, DT_CENTER | DT_VCENTER | DT_CALCRECT | DT_NOPREFIX | DT_END_ELLIPSIS);
			}
...
oScrollbar = function(themed) {
    ...
    this.setCursorButton = function() {
		...
		//gb.DrawRect(1, 0, this.cursorw-2 - 1, this.cursorh - 1, 1.0, this.color_txt & 0x44ffffff);
		gb.FillSolidRect(this.cursorw-cScrollBar.hoverWidth, 0, cScrollBar.hoverWidth, this.cursorh,scrollbar_hover_cursor_color);
		...
		//gb.DrawRect(1, 0, this.cursorw-2 - 1, this.cursorh - 1, 1.0, this.color_txt & 0x44ffffff);
		gb.FillSolidRect(this.cursorw-cScrollBar.downWidth, 0, cScrollBar.downWidth, this.cursorh,scrollbar_down_cursor_color);
...
oBrowser = function(name) {
	...
	this.get_metrics = function() {
        switch(properties.displayMode) {
            ...
            case 3:
                ...
                if(properties.displayMode == 1) {
                    this.rowHeight = 5 + cover.max_w + properties.botStampHeight;
                } else {
    ...
    this.showItemFromItemHandle = function(metadb) {
        ...
        var total_tracks = 0;
        var found = false;
        for(var a = (properties.showAllItem ? 1 : 0); a < total; a++) {
        ...
        if(found) { // scroll to album and open showlist
            if(properties.showAllItem && a == 0) a += 1;
            switch(properties.displayMode) {
                case 1:
                case 3:
    ...
    this.populate = function(is_first_populate, call_id, force_sourceMode) {
		...
		if(typeof force_sourceMode !== 'undefined') this.populate_sourceMode = force_sourceMode;
	...
	this.searchPlaylists = function(){
        ...
        for(var i = 0; i < total; i++) {
            ...
            if(this.pfound_selectionPlaylist && this.pfound_playingPlaylist) break;
        };
	}
    ...
    this.sendItemToPlaylist = function(index,index_to) {
        ...
        if(index_to>-1 && index_to>index){
			...
			for(var i = index; i <= index_to; i++) {
				...
				//from += this.groups[index].pl.Count;
			}
			playlistToInsert = this.pidx_selection;
			positionToInsert = from;
			//plman.InsertPlaylistItems(this.pidx_selection, from, toInsert, false);
		}
            ...
            if(!arrayContains(this.MultipleSelectedIndex,index)) {
				...
				playlistToInsert = this.pidx_selection;
				positionToInsert = from;				
				//plman.InsertPlaylistItems(this.pidx_selection, from, this.groups[index].pl, false);
				inserted = true;
			}
        } else {
            if(fb.IsPlaying) {
                if(plman.PlayingPlaylist == this.pidx_selection) { // playing playlist is properties.selectionPlaylist
                    plman.RenamePlaylist(this.pidx_selection, properties.playingPlaylist);
                    if(this.pfound_playingPlaylist) {
                        plman.RenamePlaylist(this.pidx_playing, properties.selectionPlaylist);
                        // 1. initialize old properties.playingPlaylist playlist
                        clearPlaylist = true;
                        //plman.SetPlaylistSelection(this.pidx_playing, items2remove, true);
                        //plman.RemovePlaylistSelection(this.pidx_playing, false);
                    } else {
                        this.pidx_playing = plman.PlaylistCount;
                        plman.CreatePlaylist(this.pidx_playing, properties.selectionPlaylist);
                    };
                    // *** insert tracks into this.pidx_playing playlist
					playlistToInsert = this.pidx_playing;
					positionToInsert = 0;		
					toInsert.InsertRange(toInsert.Count,this.groups[index].pl);					
                    //plman.InsertPlaylistItems(this.pidx_playing, 0, this.groups[index].pl, false);
					inserted = true;
					var previous_pidx=this.pidx_selection;
					this.pidx_selection = this.pidx_playing;
					this.pidx_playing = previous_pidx;
                } else {
                    // initialize true properties.selectionPlaylist playlist
                    if(this.pfound_selectionPlaylist) {
                        // clear properties.selectionPlaylist playlist content
                        clearPlaylist = true;
                        //plman.SetPlaylistSelection(this.pidx_selection, items2remove, true);
                        //plman.RemovePlaylistSelection(this.pidx_selection, false);
                    } else {
                        // create properties.selectionPlaylist playlist
                        this.pidx_selection = plman.PlaylistCount;
                        plman.CreatePlaylist(this.pidx_selection, properties.selectionPlaylist);
                    };
                    // *** insert tracks into this.pidx_selection playlist
    ...
    this.draw = function(gr) {
		...
		//if(!this.coverMask && properties.circleMode) this.DefineCircleMask(coverWidth);
        ...
        if(repaint_main || !repaintforced){
			if(this.groups.length>0){
							...
							if(this.groups[i].cover_type == null) {
								if(this.groups[i].load_requested == 0) {
									...
									if (typeof this.groups[i].cover_img !== "undefined" && this.groups[i].cover_img!==null) {
										...
										this.groups[i].cover_img = g_image_cache.hit(this.groups[i].metadb, i,this.groups[i].cachekey_album);
										if (typeof this.groups[i].cover_img !== "undefined" && this.groups[i].cover_img!==null) {
											this.groups[i].cover_type = 1;
										}
									}
								...
								if(properties.albumArtId == 4){
									...
									if(g_files.FileExists(filepath)) {
										...
										this.groups[i].save_requested = true;
										if(properties.AlbumArtFallback) this.groups[i].cover_img = g_image_cache.hit(this.groups[i].metadb, i,this.groups[i].cachekey_album);
										if (typeof this.groups[i].cover_img !== "undefined" && this.groups[i].cover_img!==null) {
											this.groups[i].cover_type = 1;
						...
						if(this.stampDrawMode) {
							...
							if((properties.displayMode == 0 || properties.displayMode == 2) && properties.drawAlternateBG) {
								if(i % 2 == 0 && !(i == this.selectedIndex || arrayContains(this.MultipleSelectedIndex,i))) {
									if(i==0) gr.FillSolidRect(ax, ay-properties.first_item_top_margin, aw, ah+properties.first_item_top_margin, g_color_alternate_row); 
									else gr.FillSolidRect(ax, ay, aw, ah, g_color_alternate_row);
								};
							} else {
								//gr.FillSolidRect(ax, ay, aw, ah, colors.normal_txt & 0x06ffffff);
							...
							if(((i == this.selectedIndex || arrayContains(this.MultipleSelectedIndex,i)) && (plman.GetPlaylistName(plman.ActivePlaylist)==properties.selectionPlaylist)) || i == g_rightClickedIndex) {
								...
								if((properties.displayMode == 0 || properties.displayMode == 2)){
									if(i==0)
										gr.FillSolidRect(ax, ay-properties.first_item_top_margin-1, aw, ah+properties.first_item_top_margin+1, selected_item_bg);
									else
										gr.FillSolidRect(ax, ay, aw, ah, selected_item_bg);								
									//top
									if(i!=0){
										//gr.FillGradRect(ax, ay, track_gradient_size, 1, 0, selected_item_line,  selected_item_line, 1.0);
										gr.FillSolidRect(ax+track_gradient_size, ay, aw-(track_gradient_size*2)-(draw_right_line?1:0), 1, selected_item_line);	
										//gr.FillGradRect(ax+aw-(track_gradient_size*2), ay, track_gradient_size, 1, 0, selected_item_line, selected_item_line, 1.0);		
									}							
									//bottom
									//gr.FillGradRect(ax, ay+ah-1, track_gradient_size, 1, 0, selected_item_line,  selected_item_line, 1.0);
									gr.FillSolidRect(ax+track_gradient_size, ay+ah-1, aw-(track_gradient_size*2)-(draw_right_line?1:0), 1, selected_item_line);		
									//gr.FillGradRect(ax+aw-(track_gradient_size*2), ay+ah-1, track_gradient_size, 1, 0, selected_item_line, selected_item_line, 1.0);
								} else if(properties.displayMode == 1)	{
									gr.FillSolidRect(hover_x, hover_y, hover_w+1, hover_h+1, selected_item_bg);																
									//if(this.activeIndex<0)
										gr.DrawRect(hover_x, hover_y, hover_w, hover_h, 1.0, selected_item_line);
								}								
							...
							if(i == this.selectedIndex) {
								txt_color1 = (properties.enableCustomColors ? colors.selected_txt : colors.normal_bg);
								txt_color2 = colors.selected_txt;
								selbg_color = colors.filter_selectedtext_bg;
							} else {
								txt_color1 = g_color_grid_txt;
								txt_color2 = g_color_grid_txt;
								selbg_color = g_color_grid_bg;
							};
						};
						...
						switch(properties.displayMode) {
							...
							case 3:
								if(!properties.showAllItem || (properties.showAllItem && i > 0) || (properties.displayMode != 3)) { 
									if(g_rightClickedIndex > -1 && properties.displayMode != 1 && properties.displayMode != 3) {
										if(g_rightClickedIndex == i && !(this.scrollbar.cursorDrag || this.scrollbar.cursorHover)) {
											if(this.stampDrawMode) {
												gr.FillSolidRect(ax, ay, width_marker_hover_item, ah, color_marker_hover_item);	
												//gr.DrawRect(ax+1, ay+1, aw-2, ah-2, 2.0, colors.filter_selectedtext_bg & 0xd0ffffff);
											} else {
												gr.FillSolidRect(ax, ay, width_marker_hover_item, ah, color_marker_hover_item);	
												//gr.DrawRect(ax + Math.round((aw - frame_w) / 2) + 1, coverTop + coverWidth - frame_h + 1, frame_w - 3, frame_h - 3, 3.0, colors.filter_selectedtext_bg & 0xddffffff);
											};
										};
									} else {
										if((i == this.activeIndex || (g_rightClickedIndex > -1 && g_rightClickedIndex == i)) && !(this.scrollbar.cursorDrag || this.scrollbar.cursorHover)) {
											if(properties.displayMode == 3) {
												...
												gr.FillSolidRect(ax, ay, hover_w, hover_h, selected_item_bg);												
												gr.DrawRect(hover_x, hover_y, hover_w, hover_h, 1.0,  selected_item_line);	
												//gr.DrawRect(ax+1, ay+1, aw-2, ah-2, 2.0, colors.filter_selectedtext_bg & 0xd0ffffff);
											} else {
												gr.FillSolidRect(ax, ay, width_marker_hover_item, ah, color_marker_hover_item);	
								...
								if(this.groups[i].cover_img) {
									...
									if(properties.showAllItem && i == 0 && total > 1) {
										...
										if(properties.displayMode == 3) {
											if(i == this.selectedIndex) {
												gr.FillSolidRect(ax + Math.round((aw - im_w) / 2), coverTop + coverWidth - properties.botGridHeight, im_w, properties.botGridHeight,g_color_gridselected_bg);
												//gr.DrawRect(ax+padding_covers-1, coverTop + coverWidth - properties.botGridHeight+1, im_w-1, properties.botGridHeight-2, 1.0, g_color_gridselected_bg);
							...
							case 2:
								...
								if(((g_rightClickedIndex > -1 && g_rightClickedIndex == i) || i == this.activeIndex) && !(this.scrollbar.cursorDrag || this.scrollbar.cursorHover)) {
									//gr.DrawRect(ax+1, ay+1, aw-2, ah-2, 2.0, colors.filter_selectedtext_bg & 0xd0ffffff);
									if(i==0)
										gr.FillSolidRect(ax, ay-properties.first_item_top_margin, width_marker_hover_item, ah+properties.first_item_top_margin, color_marker_hover_item);
									else
										gr.FillSolidRect(ax, ay, width_marker_hover_item, ah, color_marker_hover_item);						
				...
				if(main_panel_state.isEqual(0))
					gr.FillGradRect(0, wh-fading_bottom_height, ww, fading_bottom_height, 90, grad_bottom_color2,  grad_bottom_color_library_panel,1);
				else			
				...
				if(cList.search_string.length > 0) {
					...
					try {
						gr.GdiDrawText(cList.search_string, g_font.plus7, cList.inc_search_noresult?keyboard_search_txtred:keyboard_search_txt, brw.tt_x, brw.tt_y , brw.tt_w , brw.tt_h, DT_CENTER | DT_NOPREFIX | DT_CALCRECT | DT_VCENTER);
					} catch(e) {console.log(e)};
				};
				...
				if(properties.displayMode > 0) {
					...
					if(g_filterbox.inputbox.text!='' || properties.sourceMode==0) {
						var text1 = "No items";
						var text2 = "matching";											
					} else {
						var text1 = "";
					...
					if(text1!='' || properties.sourceMode==0) {
						...
						gr.FillSolidRect(this.x+Math.round(this.w/2-line_width/2),py, line_width, 1, border_color);
            ...
            if(properties.showHeaderBar) {
                ...
                // draw background part above playlist (headerbar)
				//gr.FillSolidRect(0, 0, ww, brw.y-1, colors.normal_bg);
                ...
                gr.FillSolidRect(this.x, height_top_fix1, this.w + (cScrollBar.enabled ? cScrollBar.width : 0), properties.headerBarHeight+(properties.showTagSwitcherBar ? 1 : 0), headerbar_bgcolor);
                gr.FillSolidRect(this.x, height_top_fix2+properties.headerBarHeight, this.w - this.x -((draw_right_line)?1:0), 1, headerbar_line_maincolor);
			...
			if(draw_right_line) gr.FillSolidRect(ww-1, 0, 1, wh, rightline_color);	
	...
	this.item_context_menu = function(x, y, albumIndex) {
		...
		for(var i=0; i < pl_count; i++) {
			if(i != this.playlist && !plman.IsAutoPlaylist(i)) {
				...
				if(playlist_name!=properties.selectionPlaylist) _child02.AppendMenuItem(MF_STRING, 2001 + i, plman.GetPlaylistName(i));
			};
		};
                ...
                case (idx >= 114 && idx <= 116):
                    ...
                    switch(properties.tagMode) {
                        case 1:
                            ...
                            break;
                        case 2:
							new_TFgrouping = InputBox("Enter a title formatting script.\nYou can use the full foobar2000 title formatting syntax here.\n\nSee http://tinyurl.com/lwhay6f\nfor informations about foobar title formatting.", "Artist grouping", properties.tf_groupkey_artist);
							if (!(new_TFgrouping == "" || typeof new_TFgrouping == 'undefined')) {
                            ...
                            break;
                        case 3:					
							new_TFgrouping = InputBox("Enter a title formatting script.\nYou can use the full foobar2000 title formatting syntax here.\n\nSee http://tinyurl.com/lwhay6f\nfor informations about foobar title formatting.", "Genre grouping", properties.tf_groupkey_genre);
							if (!(new_TFgrouping == "" || typeof new_TFgrouping == 'undefined')) {
...
function on_paint(gr) {
    ...
    if(!g_1x1) {
        ...
        if(fb.IsPlaying && g_wallpaperImg && properties.showwallpaper) {
            ...
            gr.FillSolidRect(0, 0, ww, wh+PanelToggleButtons_height, (properties.wallpaperblurred)?wallpaper_overlay_blurred:wallpaper_overlay);
        } else {
            if(g_wallpaperImg && properties.showwallpaper) {
                gr.DrawImage(g_wallpaperImg, 0, 0, ww, wh+PanelToggleButtons_height, 0, 0, g_wallpaperImg.Width, g_wallpaperImg.Height);
                gr.FillSolidRect(0, 0, ww, wh+PanelToggleButtons_height, (properties.wallpaperblurred)?wallpaper_overlay_blurred:wallpaper_overlay);
            } else {
				gr.FillSolidRect(0, 0, ww, wh+PanelToggleButtons_height, colors.normal_bg);
			}
        }	
...
function get_colors() {
	if(properties.darklayout){		
		...
		colors.filter_selectedtext_bg = RGBA(015,177,255,160);
		...
		btn_inactive_opacity = 130;
		btn_inactive_txt = GetGrey(140);	
		
		grad_bottom_color1 = GetGrey(0,70);
		grad_bottom_color_library_panel = GetGrey(0,70);		
		...
		headerbar_bgcolor = GetGrey(15,200);	
		...
		colors.filter_selectedtext_bg = RGBA(015,177,255,100);
		...
		btn_inactive_txt = colors.faded_txt;	
		
		grad_bottom_color1 = GetGrey(0,10);
		grad_bottom_color_library_panel = GetGrey(0,10);		
		grad_bottom_color2 = GetGrey(0,0);		
...
function on_playlist_switch() {
    if(window.IsVisible) {
		...
		g_active_playlist = plman.ActivePlaylist;
		if(brw.pidx_selection != g_active_playlist){
...
function on_notify_data(name, info) {
    switch(name) {
        ...
        case "refresh_filters":
			if(info<=properties.filterOrder && window.IsVisible){
				if(brw.current_sourceMode != 0){
