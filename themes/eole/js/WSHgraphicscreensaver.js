...
oShowList = function(parentPanelName) {
    ...
    this.getHeaderInfos = function(EvalWithMetadb) {			
		...
		if(EvalWithMetadb){
			...
			this.tf_date = ' ('+brw.groups_draw[this.idx].date+')';
			this.tf_genre = brw.groups_draw[this.idx].genre;
			this.total_tracks = this.pl.Count+(this.pl.Count>1?" tracks":" track");		
		}

		
		this.firstRowInfo = this.tf_album+this.tf_discnumber+this.tf_date;
		this.secondRowInfo = this.tf_artist;
        ...
        if(this.delta > 0) {
            ...
            if(this.y > 0 - (eval(this.parentPanelName+".h") + this.h) && this.y < eval(this.parentPanelName+".y") + eval(this.parentPanelName+".h")) {
				...
				if(properties.showlistShowCover && this.idx > -1 && brw.groups_draw[this.idx].cover_img && (this.h-this.delta_)<40){
					...
					if(typeof this.cover_img == 'undefined' || !this.cover_img) this.cover_img = FormatCover(brw.groups_draw[this.idx].cover_img_full, this.CoverSize-2*this.marginCover, this.CoverSize-2*this.marginCover, false);
