...
oShowList = function(parentPanelName) {
    ...
    this.getHeaderInfos = function(EvalWithMetadb) {			
		...
		//} else {
			...
			//Tags = TagsString.split(" ^^ ");			
			this.tf_artist = brw.groups[this.idx].artist;
			this.tf_album = brw.groups[this.idx].album;
			//this.tf_discnumber = (Tags[0]=='')?'':' - Disc '+Tags[0];
			this.tf_discnumber = '';
			if(brw.groups[this.idx].date!="?") this.tf_date = ' ('+brw.groups[this.idx].date+')';
			else this.tf_date = '';
			this.tf_genre = brw.groups[this.idx].genre;
			this.total_tracks = pl_count+(pl_count>1?" tracks":" track");		
		//}
            ...
            for(var i = start_;i < end_;i++){
				...
				if(ay >= (0 - this.rowHeight) && ay < this.y + this.h) {
					...
					try{
						...
						if(this.groups[this.groups_draw[i]].text_y+this.firstRowHeight<g_headerbar.h || this.groups[this.groups_draw[i]].text_y>g_headerbar.h) 
							gr.GdiDrawText(this.groups[this.groups_draw[i]].firstRow, g_font.plus2, colors.normal_txt, ax, this.groups[this.groups_draw[i]].text_y, this.coverRealWith, 25, (properties.centerText?DT_CENTER:DT_LEFT) | DT_TOP | DT_END_ELLIPSIS | DT_NOPREFIX);

						if(this.groups[this.groups_draw[i]].text_y + this.firstRowHeight + space_between_lines + this.secondRowHeight<g_headerbar.h || this.groups[this.groups_draw[i]].text_y>g_headerbar.h)  
							gr.GdiDrawText(this.groups[this.groups_draw[i]].secondRow, g_font.italic, colors.faded_txt, ax, this.groups[this.groups_draw[i]].text_y + this.firstRowHeight + space_between_lines, this.coverRealWith, 25, (properties.centerText?DT_CENTER:DT_LEFT) | DT_TOP | DT_END_ELLIPSIS | DT_NOPREFIX);
