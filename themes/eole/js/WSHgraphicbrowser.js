...
oFilterBox = function() {
	...
	this.on_init = function() {
		this.inputbox = new oInputbox(cFilterBox.w, cFilterBox.h, "", "Filter groups below ...", colors.normal_txt, 0, 0, colors.g_color_selected_bg, g_sendResponse, "brw", g_fsize+1, g_font.italicplus2);
	...
	this.draw = function(gr, x , y) {
        ...
        /*if(this.inputbox.edit) {
            ...
            gr.DrawRect(this.x, this.y, this.w, this.h, 1.0, border_color);
        }*/
...
oPlaylistManager = function (parentObjName) {
    ...
    this.draw = function(gr) {
		...
		gr.FillSolidRect(brw.x, 0, brw.w, wh, pm_color_overlay);
...
oRow = function(metadb,itemIndex) {
	...
	this.getTags = function(){
		...
		this.tf_discnumber = Tags[1];
		this.tf_tracknumber = Tags[2];
		this.tf_tracknumber = parseInt(this.tf_tracknumber, 10);
		if(isNaN(this.tf_tracknumber)) this.tf_tracknumber="?";
		this.tf_title = Tags[3];
		this.tf_rating = Tags[4];	
		if(this.tf_rating < 0 || this.tf_rating > 5) {
			this.tf_rating = 0;
		};
    ...
    this.draw = function(gr, x, y, w) {
		...
		if(this.tf_tracknumber>9) var select_start=4;
		else var select_start=0;
        
		if(properties.showArtistName || (properties.TFgrouping!="" && properties.TFgrouping.indexOf("artist%")==-1)) this.title_text = this.tf_title+" - "+this.tf_artist;
		else this.title_text = this.tf_title;		
		...
		if(fb.IsPlaying && fb.GetNowPlaying()!=null && this.metadb.Compare(fb.GetNowPlaying())) {//&& (nowplayinglib_state.isActive() || plman.ActivePlaylist==plman.PlayingPlaylist || brw.playlistName==globalProperties.whole_library)) {
			...
			TimeRemaining = this.tf_length_seconds - g_seconds; 
			...
			var ratio = elapsed_seconds/this.tf_length_seconds;
			if(this.tf_length=="ON AIR") {
		...
		if(properties.showRating && (!properties.showRatingSelected || this.isSelected || (properties.showRatingRated && this.tf_rating>0))) {
			rating_vpadding = 4;
		...
		if(this.tf_tracknumber=="NaN") this.tf_tracknumber="?";

		if(this.tracknumber_w==0) this.tracknumber_w = gr.CalcTextWidth(this.tf_discnumber+this.tf_tracknumber, g_font.normal)+22;
		if(!isPlaying)  gr.GdiDrawText(this.tf_discnumber+this.tf_tracknumber, g_font.normal, g_showlist.colorSchemeTextFaded, this.x-2, this.y, this.tracknumber_w, this.h, DT_RIGHT | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
        ...
        if(isPlaying && properties.AlbumArtProgressbar && properties.drawProgressBar && (cNowPlaying.flashescounter<5 || !cNowPlaying.flashEnable)) {
				...
				pt = playingText.GetGraphics();
					...
					pt.DrawString(duration, g_font.normal, albumartprogressbar_color_txt, 0 + this.w - length_w, 1, length_w, g_showlist.textHeight-g_showlist.textBot, 554696704);
		...
		// rating Stars
		if(properties.showRating && g_showlist.ratingImages && (!properties.showRatingSelected || this.isSelected || (properties.showRatingRated && this.tf_rating>0))) {
			if(this.ishover_rating && this.hover_rating>-1) {
				var rating = this.hover_rating;
			} else
				var rating = this.tf_rating;
    ...
    this.check = function(event, x, y) {
        ...
        this.ishover_rating = properties.showRating && this.ishover && (x > this.rating_x-this.rating_length/5 && x < this.rating_x + this.rating_length) && (!properties.showRatingSelected || this.isSelected || (properties.showRatingRated && this.tf_rating>0));
                ...
                if(this.ishover && y > brw.y) {
					...
					if(utils.IsKeyPressed(VK_SHIFT) && focusTrackIndex>-1) {
						...
						if(!g_dragR && this.ishover_rating) {
							...
							if(l_rating!=this.tf_rating) this.tf_rating = rateSong(l_rating,this.tf_rating, this.metadb);
            ...
            case "dblclk":
                if(this.ishover) {
					if(!nowplayinglib_state.isActive()){
						...
						plman.InsertPlaylistItems(PlaybackPlaylist, 0, brw.groups_draw[g_showlist.idx].pl);
...
oShowList = function(parentPanelName) {
    ...
    this.getHeaderInfos = function(EvalWithMetadb) {			
		...
		//if(EvalWithMetadb){
			...
			Tags = TagsString.split(" ^^ ");
			this.tf_artist = Tags[0];
			this.tf_album = Tags[1];
			this.tf_discnumber = Tags[2];
			this.tf_date = Tags[3];
			this.tf_genre = Tags[4];
			this.total_tracks = pl_count+(pl_count>1?" tracks":" track");					
		//} else {
			//TagsString = TF.showlistReduced.EvalWithMetadb(this.pl[0]);
			//Tags = TagsString.split(" ^^ ");			
			this.tf_artist = brw.groups_draw[this.idx].artist;
			this.tf_album = brw.groups_draw[this.idx].album;
			//this.tf_discnumber = (Tags[0]=='')?'':' - Disc '+Tags[0];
			this.tf_discnumber = '';
			if(brw.groups_draw[this.idx].date!="?") this.tf_date = ' ('+brw.groups_draw[this.idx].date+')';
			else this.tf_date = '';
			this.tf_genre = brw.groups_draw[this.idx].genre;
			this.total_tracks = pl_count+(pl_count>1?" tracks":" track");		
		//}

		this.firstRow = this.tf_album+this.tf_discnumber+this.tf_date;
		this.secondRow = this.tf_artist;
	...
	this.setColors = function(){		
		if(this.light_bg) {
			...
			if(properties.showListColoredOneColor || properties.showListColoredMixedColor){
				...
				this.border_color = showlist_border_color;
            ...
            if(this.y > 0 - (eval(this.parentPanelName+".h") + this.h) && this.y < eval(this.parentPanelName+".y") + eval(this.parentPanelName+".h")) {
				...
				if(properties.showlistShowCover && this.idx > -1 && brw.groups_draw[this.idx].cover_img && (this.h-this.delta_)<40){
					if(properties.CoverShadowOpacity>0) {
						if(!this.cover_shadow || this.cover_shadow==null) this.cover_shadow = createCoverShadowStack(this.coverRealSize, this.coverRealSize, cover_shadow_color,10);
...
oHeaderbar = function(name) {
    ...
    this.setButtons = function () {
		...
		if(typeof(this.FullLibraryButton) == "undefined") {
			this.FullLibraryButton = new button(this.full_library_off, this.full_library_hover, this.full_library_off);
		} else {
    ...
    this.draw = function(gr) {
		...
		//bottom line
		gr.FillSolidRect(0, this.h, ww, 1,headerbar_line_maincolor);	
		if(brw.playlistName != globalProperties.whole_library && !libraryfilter_state.isActive()) {
		...
		if(this.RightTextLength<0) {
			...
			if(brw.showFilterBox) g_filterbox.setSize(ww-this.resize_bt_w-this.rightpadding-this.MarginRight-this.mainTxtX, cFilterBox.h, g_fsize+2);					
		}
...
oBrowser = function(name) {
   ...
   timers.firstPopulate = setTimeout(function(){
    ...
    this.setSize = function(x, y, w, h) {	
		...
		if(properties.showheaderbar) {
			g_headerbar.setSize(0,0,this.w,this.headerBarHeight);
			if(this.showFilterBox) g_filterbox.setSize(ww-g_headerbar.resize_bt_w-g_headerbar.rightpadding-g_headerbar.MarginRight-g_headerbar.mainTxtX, cFilterBox.h, g_fsize+2);
		}
        ...
        while(k < this.totalTracks){
            if(properties.TFgrouping.length > 0) {
				...
				if(this.showFilterBox){
					...
					group_string = arr[0]+arr[1]+arr[2];					
				} else {
            ...
            if(string_compare != temp){
				...
				if(properties.TFgrouping.length > 0) {
					...
					this.groups[i].cachekey = process_cachekey(this.list[k],'',groupinfos[2]);					
				}
    ...
    this.draw = function(gr) {        
        if(repaint_main || repaint_f || !repaintforced){
            ...
            for(var i = start_;i < end_;i++){
                ...
                // get cover
				if(this.groups_draw[i].cover_img==null) {
				...
				if(ay >= (0 - this.rowHeight) && ay < this.y + this.h) {
					...
					if(this.groups_draw[i].cover_img!=null && typeof this.groups_draw[i].cover_img != "string") {
						...
						if(properties.CoverShadowOpacity>0 && !properties.circleMode) {
							if(!this.cover_shadow || this.cover_shadow==null) this.cover_shadow = createCoverShadowStack(this.coverRealWith, this.coverRealWith, cover_shadow_color,10);
							if(!this.cover_shadow_hover || this.cover_shadow_hover==null) this.cover_shadow_hover = createCoverShadowStack(this.coverRealWith, this.coverRealWith, cover_shadow_color_hover,10);	
						...
						if(!this.groups_draw[i].mask_applied && properties.circleMode){
							...
							width = this.groups_draw[i].cover_img.Width;
							height = this.groups_draw[i].cover_img.Height;
							coverMask = this.coverMask.Resize(width, height, 7);
							this.groups_draw[i].cover_img.ApplyMask(coverMask);
							this.groups_draw[i].mask_applied = true;
							image_to_draw = this.groups_draw[i].cover_img;
						} else {
							image_to_draw = this.groups_draw[i].cover_img;
						}					
						...
						if(!properties.circleMode)
							gr.DrawRect(ax, coverTop, this.coverRealWith-1, this.coverRealWith-1, 1.0, cover_rectline);
						else
							gr.DrawEllipse(ax+1, coverTop+1, this.coverRealWith-2, this.coverRealWith-2, 1.0, cover_rectline);

						//date drawing black
						if(properties.showdateOverCover && this.groups_draw[i].date!="?"){
							if(properties.circleMode) {
			...
			if(this.drawRightLine) gr.FillSolidRect(ww-1, 0, 1, wh, rightline_color);
            ...
            if(this.groups_draw.length == 0) { // library empty
				...
				if(this.firstInitialisation) {
					...
					gr.FillSolidRect(this.w/2 -125, py-46, 250, 1, border_color);
...
function populate_with_library_covers(start_items, str_comp_items){	
			...
			if(globalProperties.enableDiskCache) {
				if(globalProperties.load_covers_at_startup && cachekey_album!='undefined') {
					...
					if(current_item_crc_album) {				
						g_image_cache._cachelist[cachekey_album] = load_image_from_cache_direct(covers_FullLibraryList[covers_current_item], current_item_crc_album);		
					}	
				}	
				if(globalProperties.load_artist_img_at_startup && cachekey_artist!='undefined') {				
					...
					if(current_item_crc_artist) {				
						g_image_cache._cachelist[cachekey_artist] = load_image_from_cache_direct(covers_FullLibraryList[covers_current_item], current_item_crc_artist);		
					}
				}
			} else {
	...
	if(covers_current_item==covers_FullLibraryList.Count) {
		...
		//console.log("covers_array size: "+g_image_cache._cachelist.length);
		//window.NotifyOthers("cover_cache_finalized",g_image_cache._cachelist)
	}
}	
...
function on_paint(gr) {
	...
	if(!properties.showheaderbar) gr.FillSolidRect(0, 0, ww-1, brw.marginTop+brw.headerBarHeight+4,no_headerbar_top_color);	
...
function on_mouse_move(x, y, m) {
    ...
    //if(!g_ishover) {
		if(g_dragA) {
			g_avoid_on_playlist_switch = true;
			var items = brw.groups_draw[brw.clicked_id].pl; 
			brw.external_dragging = true; 
			var options = {
				show_text : false,	
				use_album_art : false,
				use_theming : false,
				custom_image : createDragImg(brw.groups_draw[brw.clicked_id].cover_img_full, 80, brw.groups_draw[brw.clicked_id].pl.Count),
			}
		...
		if(g_dragR) {
			...
			if(showlist_selected_count==items.Count) var drag_img = createDragImg(brw.groups_draw[g_showlist.idx].cover_img_full, 80,items.Count);
...
function get_colors() {
	...
	if(properties.darklayout){		
		...
		headerbar_line_maincolor = GetGrey(255,30);			
		...
		rightline_color = GetGrey(255,30);			
		...
		showlist_bg = GetGrey(0);	
		showlist_border_color = GetGrey(255,50);		
		...
		grad_bottom_color1 = GetGrey(230,90);
		...
		headerbar_line_maincolor = GetGrey(210);	
		...
		cover_ellipse_nowplaying = GetGrey(0,150);
		cover_ellipse_hover = GetGrey(0,160);			
		...
		showlist_bg = GetGrey(0,10);	
		showlist_border_color = GetGrey(210);
		showlist_color_overlay = GetGrey(0,80);	
		showlist_close_bg = GetGrey(0);
		showlist_close_icon =  GetGrey(0,165);
		showlist_close_iconhv =  GetGrey(255);	
