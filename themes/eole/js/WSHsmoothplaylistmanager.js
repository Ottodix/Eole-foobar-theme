...
function CreatePlaylist(pl_place, pl_name){      
    var QuestionString = "Give your playlist a name."
    var result = InputBox(QuestionString, "Create New Playlist", "New Playlist");
	if (result){
		if (result == "") {
...
function CreateAutoPlaylist(pl_place, pl_name, query){      
    var QuestionString = "Give your playlist a name."
    var result = InputBox(QuestionString, "Create New Autoplaylist", "New Autoplaylist");
	if (result){
		if (result == "") {
...
function DeletePlaylist(pid){
    ...
    var QuestionString = "Delete the playlist '"+parsed_tabname+"' ?";
    var result = MsgBox(QuestionString, 4, "Please confirm");
    if (result == 6) {
		if(plman.ActivePlaylist == pid){
			if(pid==plman.PlaylistCount-1) plman.ActivePlaylist = pid-1;
			else plman.ActivePlaylist = pid+1;
		}
		...
		var row = brw.getRowIdFromIdx(pid);
		plman.RemovePlaylistSwitch(pid);
		if(row < brw.rowsCount - 1) {
			brw.selectedRow = pid;
		} else if(row > 0) {
			brw.selectedRow = pid - 1;
		};
		if(updateActivePlaylist) {
			if(row < brw.rowsCount - 1) {
				plman.ActivePlaylist = pid;
			} else if(row > 0) {
				plman.ActivePlaylist = pid - 1;
			};
		};  
    }
}
...
oBrowser = function(name) {
    ...
    this.draw = function(gr) {
        ...
        if(repaint_main || !repaintforced){
                ...
                for(var i = g_start_;i <= g_end_;i++){
					...
					if(ay > this.y - properties.headerBarHeight - ah && ay < this.y + this.h) {						
                        ...
                        if(this.rows[i].idx == plman.ActivePlaylist || (i==g_rightClickedIndex && g_rightClickedIndex > -1)) {
							...
							//top
							if(i>0 ){
							gr.FillSolidRect(ax, ay, aw-1, 1, selected_item_line);		
							}
							//bottom
							gr.FillSolidRect(ax, ay+ah-1, aw-1, 1, selected_item_line);								
                        }; 
                        ...
                        if(cPlaylistManager.drag_target_id == i) {
                            if(cPlaylistManager.drag_target_id > cPlaylistManager.drag_source_id) {
                                gr.FillSolidRect(ax, ay + properties.rowHeight - 2, aw-1, 2, dragReorder_playlist_marker);
                            } else if(cPlaylistManager.drag_target_id < cPlaylistManager.drag_source_id) {
                                gr.FillSolidRect(ax, ay + 1, aw-1, 2, dragReorder_playlist_marker);
                            };
                        };
                        
                        if(g_dragndrop_status && i == g_dragndrop_targetPlaylistId) {
							if(!this.rows[i].isAutoPlaylist){
								gr.FillSolidRect(ax, ay, aw, ah, dragdrop_bg_selected_item);
								//top
								gr.FillSolidRect(ax, ay, aw, 1, dragdrop_line_selected_item);	
								//bottom
								gr.FillSolidRect(ax, ay+ah-1, aw, 1, dragdrop_line_selected_item);		
							} else {
								gr.FillSolidRect(ax, ay, aw, ah, dragdrop_disabled_item);
							}								
                        }
			...
			gr.FillGradRect(0, wh-fading_bottom_height+2, ww, fading_bottom_height, 90, grad_bottom_color2,  grad_bottom_color1,1);				 
			 
            // draw header
            if(properties.showHeaderBar) {
                var boxText = this.rows.length+" playlist"+(this.rows.length>1?"s  ":"  ");

                gr.FillSolidRect(this.x, 0, this.w - this.x -1, properties.headerBarHeight, headerbar_bgcolor);
                gr.FillSolidRect(this.x, properties.headerBarHeight, this.w - this.x -1, 1, headerbar_line_maincolor);
	...
	this.context_menu = function(x, y, id) {
        ...
        case (idx==2):
            ...
            break;
        case (idx==3):
			playlistname = InputBox("Rename the playlist: "+plman.GetPlaylistName(pl_idx), "Rename a playlist", plman.GetPlaylistName(pl_idx));
			...
			if (playlistname.length > 1 || (playlistname.length == 1 && (playlistname >= "a" && playlistname <= "z") || (playlistname >= "A" && playlistname <= "Z") || (playlistname >= "0" && playlistname <= "9"))) {
				//mytab[index].tabname = playlistname;
        ...
        case (idx==207):
            var total = plman.PlaylistCount;
            //p.playlistManager.inputboxID = -1;
            plman.CreateAutoPlaylist(total, "Top Rated", "%rating% GREATER 1 SORT DESCENDING BY %rating%", "", 1);
            plman.MovePlaylist(total, pl_idx);
            plman.ActivePlaylist = pl_idx;  
            break;			
        case (idx==210):
...
function get_colors() {
	if(properties.darklayout){		
		...
		dragdrop_bg_selected_item = GetGrey(0,190);	
		dragdrop_line_selected_item = GetGrey(255,45);	
		dragdrop_disabled_item = RGBA(255,0,0,60)
		...
		dragReorder_playlist_marker = GetGrey(255,205);
		...
		dragdrop_bg_selected_item = GetGrey(220);	
		dragdrop_line_selected_item = GetGrey(205);
		dragdrop_disabled_item = RGBA(255,0,0,60)
		...
		dragReorder_playlist_marker = GetGrey(20);
	}		
};
...
function on_drag_drop(action, x, y, mask) {
    if(y > brw.y) {
        ...
        if(brw.activeRow > -1) {
			...
			if(g_dragndrop_targetPlaylistId > -1 && brw.rows[g_dragndrop_targetPlaylistId].isAutoPlaylist) {
				action.Effect = 0;
				MsgBox("'"+plman.GetPlaylistName(g_dragndrop_targetPlaylistId)+"' is an playlist generated automatically (autoplaylist).\nYou can't manually add tracks into an autoplaylist", vb.OKOnly, "Error");
            } else if(g_dragndrop_targetPlaylistId > -1 && brw.rows[g_dragndrop_targetPlaylistId].idx>-1) {		
                action.Effect = 1;
				...
				action.ToSelect = false;		
				playlistname = InputBox("Give your playlist a name ?", "Rename a playlist", plman.GetPlaylistName(total_pl));
